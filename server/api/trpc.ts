 ...
 *
 * - Attempts to retrieve the current user session via `getServerSession()`.
...
const authenticationMiddleware = t.middleware(async ({ next }) => {
  const { data: session, error: sessionError } = await tryCatch(getServerSession());
...
const adminMiddleware = t.middleware(async ({ next, ctx }) => {
  ...
  if (session.user.role !== "admin") {
    throw new TRPCError({ code: "FORBIDDEN", message: "User not allowed" });
  }
