  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4">
      ...
      <div className="space-y-2 pb-2 text-center">
        <h1 className="text-2xl font-bold text-foreground lg:text-4xl">Quiz History</h1>
        <p className="text-sm leading-2 text-muted-foreground">Track your learning progress and achievements</p>
      </div>
          ...
          <Card key={label} className="py-2! shadow-sm transition-all duration-300 hover:shadow-md">
            <CardContent className="flex flex-col items-center justify-center gap-2 p-3 text-center sm:flex-row sm:items-center sm:gap-4 sm:p-4 sm:text-left">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-foreground sm:text-2xl">{value}</p>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Tabs value={activeTab} onValueChange={value => setActiveTab(value as TabType)}>
        ...
        <TabsContent value="taken">
          <Card className="border border-border bg-card/80 shadow-lg backdrop-blur-sm">
            <CardHeader>
              <CardTitle className="text-2xl text-foreground">Recent Quiz Results</CardTitle>
                      ...
                      takenHistory.quizzesTaken.map(quiz => {
                        ...
                        return (
                          <Link href={`/quiz/results/${quiz.id}`} key={quiz.id} className="group block">
                            <div className="rounded-xl border border-border/50 bg-card p-4 transition-all duration-200 group-hover:bg-muted/30 hover:border-border hover:shadow-md">
                              {/* Mobile Layout */}
                              <div className="flex flex-col gap-4 md:hidden">
                                ...
                                {/* Topic and Author */}
                                <div>
                                  <h3 className="mb-1 text-lg leading-tight font-semibold text-foreground">
                                    {quiz.Quiz.topic}
                                  </h3>
        ...
        <TabsContent value="created">
          <Card className="border border-border bg-card shadow-lg backdrop-blur-sm">
            <CardHeader>
              <CardTitle className="text-2xl text-foreground">Created Quizzes</CardTitle>
                  ...
                  <div className="space-y-4">
                    {createdHistory._count.quizzesCreated === 0 ? (
                      ...
                      createdHistory.quizzesCreated.map(quiz => (
                        <div key={quiz.id}>
                          <div className="flex flex-col items-start justify-between gap-4 rounded-lg border border-border/50 bg-card p-4 shadow-md transition-colors hover:bg-muted/70 sm:flex-row sm:items-center">
                            {/* Left side – untouched */}
                            <div className="flex w-full items-center gap-4">
                              <div className="flex items-center justify-center gap-2 rounded-full border border-border px-3 py-2 font-semibold">
